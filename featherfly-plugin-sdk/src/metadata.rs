...
pub const EVENT_DOCS: &[EventDoc] = &[
    ...
    EventDoc {
        event: PluginEvent::ConfigUpgraded,
        name: "config.upgraded",
        summary: "On-disk config.yml was rewritten with new default keys.",
        when: "Daemon startup detects missing config keys or legacy migration rewrite.",
        payload: "JSON: `{ path }`",
        cancelable: false,
        details: "Serde defaults fill missing keys at runtime; this event fires when those keys are persisted to disk.",
        use_cases: &[
            "Audit config drift after upgrades",
            "Backup config before rewrite",
        ],
        register_example: "hook!(host, PluginEvent::ConfigUpgraded, on_config_upgraded);",
        handler_example: JSON_EVENT_HANDLER,
    },
    EventDoc {
        event: PluginEvent::HostingReconciled,
        name: "hosting.reconciled",
        summary: "Startup hosting reconcile finished.",
        when: "hosting.reconcile_on_startup completes site/mail/FTP/shared stack sync.",
        payload: "JSON: `{ sites_total, sites_started, mail_synced, ftp_synced, shared_stack }`",
        cancelable: false,
        details: "Summary counters reflect what succeeded during boot reconcile.",
        use_cases: &["Panel boot status", "Alert when shared stack fails to sync"],
        register_example: "hook!(host, PluginEvent::HostingReconciled, on_reconcile);",
        handler_example: JSON_EVENT_HANDLER,
    },
    EventDoc {
        event: PluginEvent::SharedStackSynced,
        name: "hosting.shared_stack_synced",
        summary: "Shared mail, FTP, phpMyAdmin, and error-page stack reconciled.",
        when: "sync_shared_stack succeeds with hosting.shared_services enabled.",
        payload: "JSON: `{ mail, roundcube, sftp, ftp, phpmyadmin, database_servers }`",
        cancelable: false,
        details: "Boolean flags indicate which shared services were touched during sync.",
        use_cases: &["Stack health dashboards", "Trigger external monitoring"],
        register_example: "hook!(host, PluginEvent::SharedStackSynced, on_shared_stack);",
        handler_example: JSON_EVENT_HANDLER,
    },
    EventDoc {
        event: PluginEvent::SharedDatabaseServersReady,
        name: "hosting.shared_database_servers_ready",
        summary: "Shared MySQL/Postgres/Redis/MongoDB containers are running.",
        when: "ensure_shared_database_servers completes during shared stack sync or DB create.",
        payload: "JSON: `{ mysql, postgres, redis, mongodb, site_networks_connected }`",
        cancelable: false,
        details: "Containers use featherfly-mysql, featherfly-postgres, featherfly-redis, featherfly-mongodb names.",
        use_cases: &["DB monitoring", "Panel infrastructure overview"],
        register_example: "hook!(host, PluginEvent::SharedDatabaseServersReady, on_shared_db);",
        handler_example: JSON_EVENT_HANDLER,
    },
    EventDoc {
        event: PluginEvent::PhpMyAdminProvisioned,
        name: "hosting.phpmyadmin_provisioned",
        summary: "Shared phpMyAdmin container reconciled with Traefik routes.",
        when: "ensure_shared_phpmyadmin succeeds during shared stack sync.",
        payload: "JSON: `{ container_id, site_count }`",
        cancelable: false,
        details: "Routes phpmyadmin.{domain} per site to the shared PMA container.",
        use_cases: &["Web DB admin links in panel", "Route audit"],
        register_example: "hook!(host, PluginEvent::PhpMyAdminProvisioned, on_pma);",
        handler_example: JSON_EVENT_HANDLER,
    },
    EventDoc {
        event: PluginEvent::ErrorPagesReady,
        name: "hosting.error_pages_ready",
        summary: "Branded error pages and Traefik middleware are in place.",
        when: "ensure_error_page_files and error nginx container sync succeed.",
        payload: "JSON: `{ pages_directory, container_id }`",
        cancelable: false,
        details: "Serves 404/500/502/503 HTML via featherfly-errors nginx sidecar.",
        use_cases: &["Custom error page CDN sync", "Branding verification"],
        register_example: "hook!(host, PluginEvent::ErrorPagesReady, on_error_pages);",
        handler_example: JSON_EVENT_HANDLER,
    },
    EventDoc {
        event: PluginEvent::TraefikProvisioned,
        name: "proxy.traefik_provisioned",
        summary: "Traefik reverse proxy container was created, restarted, or confirmed running.",
        when: "Docker bootstrap ensure_traefik completes with a non-skipped result.",
        payload: "JSON: `{ result, image, network, tls_enabled }`",
        cancelable: false,
        details: "result is created, restarted, or already_running.",
        use_cases: &["Proxy health monitoring", "TLS rollout tracking"],
        register_example: "hook!(host, PluginEvent::TraefikProvisioned, on_traefik);",
        handler_example: JSON_EVENT_HANDLER,
    },
    EventDoc {
        event: PluginEvent::HostingImagesPulled,
        name: "hosting.images_pulled",
        summary: "Hosting stack container images were pulled on reconcile.",
        when: "auto_pull_images or auto_update_images runs during hosting reconcile.",
        payload: "JSON: `{ image_count, images }`",
        cancelable: false,
        details: "Includes mail, FTP, DB, phpMyAdmin, error-pages, and Traefik when enabled.",
        use_cases: &["Image update audit", "Bandwidth monitoring"],
        register_example: "hook!(host, PluginEvent::HostingImagesPulled, on_images);",
        handler_example: JSON_EVENT_HANDLER,
    },
    EventDoc {
        event: PluginEvent::PluginConfigMutated,
        name: "plugins.config_mutated",
        summary: "A config.mutate hook changed the raw daemon config.",
        when: "After the config mutation pipeline returns bytes different from the file on disk.",
        payload: "JSON: `{ hook_count, input_len, output_len }`",
        cancelable: false,
        details: "Emitted once for the config mutation pipeline when at least one registered config hook changes the YAML bytes before parsing.",
        use_cases: &[
            "Audit plugin-driven config changes",
            "Track dynamic defaults",
        ],
    ...
    EventDoc {
        ...
        summary: "A request hook returned an HTTP response before the route handler ran.",
        when: "request.intercept or middleware.inject returns REQUEST_RESPOND.",
        payload: "JSON: `{ phase, method, path, status }`",
        cancelable: false,
        details: "The request did not continue to later hooks or the Axum handler; the plugin-provided response was sent to the client.",
        use_cases: &["Custom auth audit", "Rate-limit dashboards"],
    ...
    EventDoc {
        ...
        summary: "A json.response hook changed a JSON response body.",
        when: "The JSON response mutation pipeline returns bytes different from the handler response.",
        payload: "JSON: `{ method, path, input_len, output_len }`",
        cancelable: false,
        details: "Emitted after response body hooks run and before json.actions hooks are applied.",
        use_cases: &["Observe response rewriting", "Debug panel extension hooks"],
        register_example: "hook!(host, PluginEvent::PluginJsonResponseMutated, on_json_response);",
        handler_example: JSON_EVENT_HANDLER,
    },
    EventDoc {
        event: PluginEvent::PluginJsonActionsMutated,
        name: "plugins.json_actions_mutated",
        summary: "A json.actions hook changed an API actions array.",
        when: "The JSON actions mutation pipeline returns bytes different from the original actions array.",
        payload: "JSON: `{ method, path, input_len, output_len }`",
        cancelable: false,
        details: "Emitted before the mutated actions array is merged back into the top-level response object.",
        use_cases: &["Audit action injection", "Debug panel workflows"],
    ...
    EventDoc {
        ...
        summary: "A plugin-registered HTTP route handled a request.",
        when: "A matching route.register handler returns ROUTE_OK.",
        payload: "JSON: `{ plugin, method, path, status }`",
        cancelable: false,
        details: "The response status is the status code written by the plugin route handler.",
        use_cases: &["Plugin route analytics", "Audit custom API usage"],
    ...
    EventDoc {
        ...
        summary: "A plugin-registered HTTP route failed before producing a response.",
        when: "The request body could not be read or the route handler returns an error code.",
        payload: "JSON: `{ plugin, method, path, error }`",
        cancelable: false,
        details: "FeatherFly returns 400 for body read failures and 500 for plugin handler errors.",
        use_cases: &["Alert on plugin route errors", "Debug custom API handlers"],
