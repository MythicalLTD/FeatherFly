use crate::{
    routes::GetState,
    utils::{
        plugin_events::{self, ErrorPayload, UpgradeStartedPayload},
...
pub async fn post(state: GetState, axum::Json(data): axum::Json<Payload>) -> ApiResponseResult {
    ...
    let client = match crate::utils::update::http_client() {
        Ok(client) => client,
        Err(_) => {
    ...
    plugin_events::emit_json(
        &state.plugins,
        featherfly_plugin_sdk::PluginEvent::UpgradeStarted,
    ...
    tokio::spawn(async move {
        ...
        .await
        {
            Ok(()) => {
                plugin_events::emit_json(
                    &plugins.plugins,
                    featherfly_plugin_sdk::PluginEvent::UpgradeCompleted,
            ...
            Err(err) => {
                ...
                plugin_events::emit_json(
                    &plugins.plugins,
                    featherfly_plugin_sdk::PluginEvent::UpgradeFailed,
                    &ErrorPayload {
                        error: err.to_string(),
                    },
                );
